...
async function callback(req, res, next) {
  try {
    ...
    if (withingsError || !code) {
      return res.redirect(`${frontendBase}/profile?withings=denied`);
    }
    ...
    try {
      ...
      return res.redirect(`${frontendBase}/profile?withings=error`);
    }
    ...
    res.redirect(`${frontendBase}${returnTo}?withings=connected`);
  } catch (err) {
    next(err);
  }
}
...
function getStatus(req, res, next) {
  try {
    const row = db.prepare(
      'SELECT withings_access_token, withings_token_expires_at FROM user_profiles WHERE user_id = ?'
    ).get(req.userId);
