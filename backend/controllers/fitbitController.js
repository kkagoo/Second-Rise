...
async function callback(req, res, next) {
  try {
    ...
    if (fitbitError || !code) {
      return res.redirect(`${frontendBase}/profile?fitbit=denied`);
    }
    ...
    try {
      ...
      return res.redirect(`${frontendBase}/profile?fitbit=error`);
    }
    ...
    res.redirect(`${frontendBase}${returnTo}?fitbit=connected`);
  } catch (err) {
    next(err);
  }
}
...
function getStatus(req, res, next) {
  try {
    const row = db.prepare(
      'SELECT fitbit_access_token, fitbit_token_expires_at FROM user_profiles WHERE user_id = ?'
    ).get(req.userId);
